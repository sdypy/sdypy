## Purpose
The contract governing testing and CI for the `sdypy` core package and all six first-level packages — the `pypi_artifacts` marker policy that keeps CI green while preserving a local release gate, the required shape of the core GitHub Actions workflows, the minimum real functional test baseline per first-level package, the cross-package interop suite, and the requirement that all seven fork CIs demonstrate green. Established by the `establish-testing-ci-baseline` change.
