## Purpose
The `documentation` capability defines the contract for all documentation artefacts across the SDyPy core umbrella and its six first-level sibling packages (`EMA`, `io`, `FRF`, `excitation`, `view`, `model`). It covers the no-foreign-project-residue rule, the unified `pydata-sphinx-theme` standard, the umbrella RTD site structure, per-package docs baseline, README format, SEP rendering, autodoc for own-code packages, standardised `readthedocs.yaml`, and conformance checks.
